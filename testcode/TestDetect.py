...
def Detect_Lane(image, plot_image=False,
    ...
    left_top = (image_shape[1] / 2 -50, 450)
    right_top = (image_shape[1] / 2 +50, 450)
...
def process_image(image):
    ...
    try:
        ...
        #     plt.axis('off')
        #     plt.show()
        return None, result2, lines_image
            
    return connected_lines, result1, lines_image
...
def Process_Video(input_video_path):
    ...
    # Hiển thị video trong một cửa sổ pop-up
    while True:
        vertices = np.array([[(0,frame.shape[0]),(frame.shape[1] / 2 -50, 450), (frame.shape[1] / 2 +50, 450), (frame.shape[1],frame.shape[0])]], dtype=np.int32)
        lines_edges = cv2.polylines(frame,vertices, True, (0,0,255), 10)
        connected_lines, result, lines_image = process_image(frame)
        try:
            result = cv2.resize(result, (width, height))
            print(pr(result))
        except Exception as e:
            print("Cannot detect")
        cv2.imshow('Processed Video1', result)
        cv2.imshow('Processed Video', lines_edges)
...
input_video_path = os.path.join('lane/', 'test1.mp4') # os.path.join(test_video_path, input('Enter test video name: '))
